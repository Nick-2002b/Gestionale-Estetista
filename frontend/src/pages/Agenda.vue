  ...
  <div class="relative flex-1 flex flex-col h-full min-h-0">
    ...
    <div v-if="contextMenu.visible" :style="{ top: `${contextMenu.y}px`, left: `${contextMenu.x}px` }"
      ...
      <button @click="askDelete"
        class="text-left w-full p-2 text-sm text-red-600 hover:bg-red-50 flex items-center gap-2 transition-colors cursor-pointer">
        <svg xmlns="http://www.w3.org/2000/svg" width="15" height="18" viewBox="0 0 24 24" fill="none"
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between shrink-0 my-4">
      <div class="flex p-1 items-center rounded-full shadow-sm border border-gray-200 space-x-1 me-3">
        <button @click="goPrev"
          class="p-2 text-gray-600 transition-colors rounded-full hover:bg-pink-light cursor-pointer">
      ...
      <button @click="goToday"
        class="px-4 py-2 bg-surface border border-gray-200 rounded-full shadow-sm hover:bg-pink-light text-sm font-medium text-gray-700 transition-colors cursor-pointer">Oggi</button>
      ...
      <div class="flex bg-gray-100 border border-gray-200 rounded-full shadow-sm">
        <div class="flex p-1 space-x-2">
          <button v-for="viewMode in viewModes" :key="viewMode.value" @click="changeView(viewMode.value)"
            :class="['px-4 py-1 text-sm font-medium rounded-full transition-all cursor-pointer', currentView === viewMode.value ? 'bg-surface shadow-sm text-gray-900' : 'text-gray-500 hover:bg-gray-200']">
            {{ viewMode.label }}
          </button>
        </div>
      </div>
        ...
        <FullCalendar ref="calendarRef" :options="finalCalendarOptions" class="absolute inset-0">
          <template v-slot:dayHeaderContent="arg">
            <div class="flex flex-col items-center justify-center py-2">
              ...
              <span
                :class="['font-semibold w-8 h-8 flex items-center justify-center rounded-full transition-colors', arg.isToday ? 'bg-pink  shadow-sm' : 'text-gray-800']">
                {{ arg.date.getDate() }}
              </span>
            </div>
          </template>
          ...
          <template v-slot:eventContent="arg">
            ...
            <div v-if="((arg.event.end?.getTime() || 0) - (arg.event.start?.getTime() || 0)) / 60000 <= 15"
            ...
            <div v-else @contextmenu.prevent="openContextMenu($event, arg.event)"
              ...
              <div v-if="arg.event.extendedProps.notes" class="text-xs text-gray-500 text-left w-full truncate mt-auto">
                Note: {{ arg.event.extendedProps.notes }}</div>
            </div>
          </template>
        </FullCalendar>
      </div>
