  ...
  <div class="flex h-screen overflow-hidden bg-surface font-sans">
    <aside class="hidden md:flex w-64 shrink-0 bg-surface border-r border-gray-200 flex-col overflow-y-auto z-20">
      ...
      <nav class="flex-1 px-4 py-6 space-y-2">
        <RouterLink v-for="item in navigation" :key="item.name" :to="item.path" class="flex px-4 py-3 text-sm font-medium rounded-full transition-colors" :class="[route.path === item.path ? 'bg-pink-50 ' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900']">
          <svg v-if="item.icon === 'calendar'" :class="['w-5 h-5 mr-3', isActiveRoute(item.path) ? 'text-pink-400' : 'text-gray-400']" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            <path stroke-linecap="round" stroke-linejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
          </svg>
          <svg v-if="item.icon === 'users'" :class="['w-5 h-5 mr-3', isActiveRoute(item.path) ? 'text-pink-400' : 'text-gray-400']" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            <path stroke-linecap="round" stroke-linejoin="round" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" />
          </svg>
          <svg v-if="item.icon === 'sparkles'" :class="['w-5 h-5 mr-3', isActiveRoute(item.path) ? 'text-pink-400' : 'text-gray-400']" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
          ...
          <transition name="dropdown-fade">
            <div v-if="showDropdown" class="absolute origin-top right-0 mt-3 p-2 bg-white rounded-lg shadow-lg border border-gray-100 w-48 z-50">
              ...
              <div class="px-1 mt-1 hover:bg-pink-100 flex items-center justify-between rounded-sm transition-colors" @click="handleLogout">
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="mr-1 h-4 w-4" aria-hidden="true">
