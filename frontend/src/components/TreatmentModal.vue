...
const handleCreateCategory = async () => {
  ...
  try {
    ...
    categoryErrorMessage.value = getApiErrorMessage(error, "Errore durante la creazione della categoria.");
  }
};
</script>
    ...
    <div class="space-y-4">
      ...
      <div>
        ...
        <div v-if="showNewCategoryForm" class="mt-3 rounded-lg border border-dashed border-pink-200 bg-pink-50/60 p-3 space-y-3">
          <div v-if="categoryErrorMessage" class="rounded-lg border border-pink-light px-4 py-3 text-sm bg-background text-gray-800">
